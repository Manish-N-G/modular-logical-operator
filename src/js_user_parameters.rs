...
#[wasm_bindgen]
pub fn enter_input_js(input: &str) -> String {
...
#[wasm_bindgen]
pub fn input_expression_client_js(input:&str) -> bool {
...
pub fn assign_user_leftparameters_set(list:Regex, expx:&str) ->Vec<&str> {
...
pub fn count_my_input_param_js(input:&str)-> i8{
    ...
    let my_array: Vec<&str> = assign_user_leftparameters_set(list, input);
    my_array.len() as i8
}


//creates a hash and assigns input array to variables from parameter_input for js
pub fn assign_user_parameters(list: Regex, array: Vec<f64>, expx: &str) -> LinkedHashMap<String, f64> {
...
#[wasm_bindgen]   
pub fn assign_and_create_param_array_js(input_str: &str, array:Vec<f64>) ->Array {
    let my_array:Vec<f64> = Vec::from(array); 
    let list = RegExpEnum::HashParameterLeftOnly.check_reg_ex().unwrap();
    let my_hash: LinkedHashMap<String, f64> = assign_user_parameters(list, my_array ,input_str);  
    let map_array: Array = send_linked_hash_to_vecter(my_hash);
    map_array
}

// Send array of variable names as String 
#[wasm_bindgen]   
pub fn send_var_array_names_js(input_str: &str) ->Array {
 
    let list = RegExpEnum::HashParameterLeftOnly.check_reg_ex().unwrap();
    let my_array: Vec<&str> = assign_user_leftparameters_set(list, input_str);  
    let js_array = js_sys::Array::new();
    for str_ref in my_array {
        js_array.push(&JsValue::from_str(str_ref));
    }
    js_array.into()
}
...
//fn to check if int
#[wasm_bindgen] 
pub fn check_int_js(input_srt:&str) -> bool {
    match input_srt.trim().parse::<f64>() {
        Ok(_) => return true,
        Err(_) => false,
    }
}
...
pub fn send_linked_hash_to_vecter<T: ToJsValue>(para_hash: LinkedHashMap<String, T>) -> Array {
...
pub fn get_logical_hash_array_js(input_str: &str) -> Array {
    ...
    send_linked_hash_to_vecter(logical_hash)
}
...
pub fn get_bool_hash_array_js(input_str: &str, array: Vec<f64>) -> Array {
    ...
    send_linked_hash_to_vecter(bool_hash)
}

// fn to get an array of the main result modularly and final result
#[wasm_bindgen]
pub fn result_function_js(input_str: &str, array: Vec<f64>)-> Array {
    let my_array: Vec<f64> = Vec::from(array);
    let my_hash = create_user_parameters_hash(my_array, input_str);
    let logical_hash = create_logical_hash(input_str);
    let bool_hash = create_bool_hash(input_str, &my_hash);
...
fn create_user_parameters_hash(array: Vec<f64>, input_str: &str) -> LinkedHashMap<String, f64> {
    let list = RegExpEnum::HashParameterLeftOnly.check_reg_ex().unwrap();
    assign_user_parameters(list, array, input_str)
}
